...
def obtain_moe_for_rotated(calibrate, iou_thr):
    # model_names = ['rtmdet', 'lsk', 'orcnn', 'gliding_vertex']
    model_names = ['orcnn', 'gliding_vertex']
    from_scratch = True
    det_paths = []
    
    if calibrate:
        target_path = '/home/lx/mocae/mocae_rotated_object_detection/work_dirs/mocae_og2/Task1/'
    else:
        target_path = '/home/lx/mocae/mocae_rotated_object_detection/work_dirs/vanilla_moe/Task1/'
        
    image_files = '/home/lx/mocae/mocae_rotated_object_detection/test_images.npy'
        
    for model_name in model_names:
        det_paths.append('/home/lx/mocae/mocae_rotated_object_detection/work_dirs/' + model_name + '_ms/Task1/')
    ...
    if calibrate:
        ...
        for model_name in model_names:
            print(model_name)
            calibration_file = "/home/lx/mocae/mocae_rotated_object_detection/" + model_name + "/calibrators/IR_class_agnostic_finaldets458.pkl"
    ...
    for fname in inputs:
        ...
        if calibrate:
            ...
            for j in range(len(det_paths)):
                ...
                # print(f"Calibrator length: {len(calibrator)}, j: {j}, init_idx: {init_idx}, end_idx: {end_idx}")
